...
Kept out of `edupilot.api` so the HTTP layer contains routing and nothing
...
from edupilot.ingestion.chunking import ChunkingConfig
from edupilot.retrieval import (
    BM25Encoder,
    Embedder,
    EmbeddingCache,
    HybridRetriever,
...
from edupilot.security import UserStore
...
class Services:
                ...
                if not self._sparse_loaded:
                    ...
                    if self._sparse is None:
                        logger.warning(
                            ...
                            "until you run `edupilot-reindex --rebuild`",
                            SPARSE_ENCODER_PATH,
                        )
        return self._sparse
    ...
    def pipeline(self):
        if self._pipeline is None:
            with self._lock:
                if self._pipeline is None:
                    from edupilot.agents import EduPilotPipeline
                    from edupilot.llm import call_llm
    ...
    def health(self) -> dict:
        ...
        from edupilot.core.config import AVAILABLE_MODELS, DEFAULT_MODEL, GROQ_MODELS
